...
var GFixedFunc = make(map[string]Fixed, 8) // key的格式为Package.Func
...
// AllCallCase 调用函数的N种情况  对函数调用使用todo适配，并返回是否适配成功
...
// FixContext 主要用于将调用链里面的nil替换为ctx,判断填充父函数的行参context.Context并在源头函数生成Context的起点
